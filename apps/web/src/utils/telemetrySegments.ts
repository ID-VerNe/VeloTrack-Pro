...
export type { PauseCluster } from './pauseClusterDetector';
...
export function analyzeRideTelemetry(
  ...
  // 1-4. 识别轨迹中真实的静止停顿区并聚类为独立停顿事件
  const { pauseClusters, cumulativeDistances, stepDistances } = detectPauseClusters(
    routeCoordinates,
    totalPausedSecs,
    totalElapsedSecs
  );

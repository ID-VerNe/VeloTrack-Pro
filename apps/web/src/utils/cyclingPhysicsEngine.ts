 ...
 *
 * 从 packages/api/src/utils/cyclingPhysicsEngine.ts 平移（纯数学，无运行时依赖）。
 * 前端 AI 服务直接调用，保证 tool-calling 计算结果与后端一致。
 */
...
export interface GearAnalysisResult {
  ...
  gear_table: Array<{
    ...
    gear_inches: number;
    speed_at_cadence_kmh?: number;
    cadence_for_target_speed?: number;
    is_sweetspot_recommended?: boolean;
    sweetspot_speed_range_kmh: { min: number; max: number };
  }>;
